...
def _get_4ps_feat(cc_match, output):
  ...
  if not(isinstance(output, dict)):
    cc_match = torch.where(cc_match<feat.shape[1], cc_match, (feat.shape[0]-1)* torch.ones(cc_match.shape).to(torch.int64))
    cc_match = torch.where(cc_match>=0, cc_match, torch.zeros(cc_match.shape).to(torch.int64))
