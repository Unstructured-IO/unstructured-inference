...
def _topk(scores, K=40):
    ...
    topk_inds = topk_inds % (torch.Tensor([height]).to(torch.int64) * torch.Tensor([width]).to(torch.int64))
    topk_ys   = (topk_inds / torch.Tensor([width])).int().float()
    topk_xs   = (topk_inds % torch.Tensor([width]).to(torch.int64)).int().float()
...
def find4ps(bbox, x, y):
    xs = torch.Tensor([bbox[0],bbox[2],bbox[4],bbox[6]])
    ys = torch.Tensor([bbox[1],bbox[3],bbox[5],bbox[7]])
